...
class YouchieAnalogClock(Renderer):
	...
	def applySkin(self, desktop, parent):
		...
		for (attrib, what) in self.skinAttributes:
			...
			elif attrib == 'sizeWidth':
				try:
					self.sizeWidth = int(what)
				except: pass
			else:
	...
	def draw_line(self, x0, y0, x1, y1):
		...
		for x in range(int(x0), int(x1 + 1)):
			if steep:
				self.instance.fillRect(eRect(int(y), int(x), int(self.sizeWidth), int(self.sizeWidth)), self.fColor)
			else:          
				self.instance.fillRect(eRect(int(x), int(y), int(self.sizeWidth), int(self.sizeWidth)), self.fColor)
			error = error + deltay
			if error > 0:
				y = y + ystep
				error = error - deltax
